 ...
 * Changes added by Dragos 
 * https://github.com/themsk666/Plex-Export-Dark-Mode
 *
...
const PLEX = {
  ...
  display_sections_list() {
    ...
    PLEX.section_display_order.forEach(key => {
      ...
      sectionListHTML += `<li data-section="${section.key}" 
      class="${section.type}"><span>${section.title}</span></li>`;
  ...
  generate_item_content() {
   ...
   popupHeader +=`${ratingTag}</div>`
    ...
    const popupSidebar = `<div id="popup-sidebar"><img src="${imgThumb}" /></div>`;
   ...
   if (PLEX.current_item.duration > 0) {
    ...
    if (PLEX.current_item.content_rating !== false)
     popupContent += `<li><strong>Rated:</strong> ${PLEX.current_item.content_rating}</li>`;
